...
interface NovaContaDialogProps {
  ...
  onSalvar: (dados: NovaContaInput) => void;
}
...
export function NovaContaDialog({
  ...
  const [dataVencimento, setDataVencimento] = useState(() => dataHojeISO());
  ...
  async function extrairDoArquivo(file: File) {
    ...
    try {
      ...
      if (dados.data && /^\d{4}-\d{2}-\d{2}$/.test(dados.data)) {
        setDataVencimento(dados.data);
        preenchidos.push("data");
      }
      if (dados.categoria && CATEGORIAS_VALIDAS.has(dados.categoria)) {
      ...
      if (preenchidos.length > 0) {
        ...
        toast.warning("Documento processado, mas nenhum dado pôde ser extraído.");
      }
    } catch (erro) {
      console.error(erro);
    ...
    <Dialog open={open} onOpenChange={(o) => (o ? onOpenChange(true) : handleClose())}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Preencha os campos manualmente ou anexe o documento — a IA extrai os
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label className="flex items-center gap-1.5">
              ...
              Anexar documento (preenchimento automático)
            </Label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              <Label htmlFor="data-vencimento">
                Vencimento{" "}
                {campoExtraido.includes("data") && <BadgeIA />}
              </Label>
              <Input
                id="data-vencimento"
                type="date"
